...
class LearnQuizComponent extends StatefulWidget {
  ...
  const LearnQuizComponent({super.key, required this.questions, required this.onFinish});
...
class _LearnQuizComponentState extends State<LearnQuizComponent> {
  late List<LearnSectionQuestionModel> questions;
  ...
  @override
  void didUpdateWidget(covariant LearnQuizComponent oldWidget) {
  ...
  /// --- Getters ---
  LearnSectionQuestionModel get currentQuestion => questions[questionIndex];
  List<LearnSectionAnswerModel> get currentAnswers => currentQuestion.answers ?? [];
  ...
  Widget continueButton(NextTaskState state, LearnQuestionAnswerState answerState) =>
      Button.primary(
          onTap: continueAfterAnswer,
          text: 'Continue',
          isAvialable: state.isCurrentTaskCompleted && !answerState.status.isLoading);

  Widget get view => BlocBuilder<LearnQuestionAnswerBloc, LearnQuestionAnswerState>(
      bloc: answerBloc,
      builder: (context, answerState) => BlocBuilder<NextTaskBloc, NextTaskState>(
          builder: (context, state) => Column(children: [
                ...
                progressHeader,
                const SizedBox(height: 24),
                LearnQuestionsCard(questionText: currentQuestion.title),
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<LearnQuestionAnswerBloc, LearnQuestionAnswerState>(
        bloc: answerBloc,
